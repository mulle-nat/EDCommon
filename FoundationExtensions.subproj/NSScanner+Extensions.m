...
- (BOOL)scanUpToClosingBracketIntoString:(NSString **)stringRef
{
    ...
    if((location = [self scanLocation]) == 0)
        [NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: Attempt to scan up to a closing bracket even though no bracket has been scanned.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
